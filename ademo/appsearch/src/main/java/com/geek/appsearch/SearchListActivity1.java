...
//        getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_VISIBLE);
        get_flowlayout();
        mList = new ArrayList<>();
        presenter = new SNewSearchListPresenter();
...
//        });
        mAdapter2.setOnItemChildClickListener(new BaseQuickAdapter.OnItemChildClickListener() {
            ...
            public void onItemChildClick(BaseQuickAdapter adapter, View view, int position) {
                SNewListBean1.ListBean bean = (SNewListBean1.ListBean) adapter.getItem(position);
                int i = view.getId();
//                Toasty.normal(SearchListActivity.this, bean.getBookName() + "    " + position).show();
                focus = 1;
                String title = String.valueOf(Html.fromHtml(bean.getTitle()));
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    edit_query1.setText(Html.fromHtml(title, Html.FROM_HTML_MODE_COMPACT));
                } else {
                    edit_query1.setText(Html.fromHtml(title));
                }
//                edit_query1.setText(bean.getTitle());
                SearchBean bean2 = new SearchBean(title, title, false);
                add_history(bean2);
...
//                }
        ...
        mAdapter4.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                ...
                if (bean != null) {
                    try {
                        if (bean.getExt().getAppUrl() != null && !"".equals(bean.getExt().getAppUrl())) {
                            LogUtils.e("AppUrl" + bean.getExt().getAppUrl());
                            HiosHelperNew.resolveAd(SearchListActivity1.this, SearchListActivity1.this, bean.getExt().getAppUrl());
                        }
                    } catch (Exception e) {
...
//                HiosHelper.resolveAd(SearchListActivity1.this, SearchListActivity1.this, bean.getExt().getAppUrl());
            }
        });
        ...
        mAdapter3.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                ...
                HiosHelper.resolveAd(SearchListActivity1.this, SearchListActivity1.this, bean.getHios());
            }
        });
    ...
    private void findview() {
        ...
        //
        recyclerView4.setLayoutManager(new GridLayoutManager(this, 1, RecyclerView.VERTICAL, false));
        mAdapter4 = new SearchListAdapter4(R.layout.activity_search_list_item4);
        recyclerView4.setAdapter(mAdapter4);
        niubiEmptyView3 = new NiubiEmptyViewNew();
        niubiEmptyView3.bind(this, recyclerView4, mAdapter4);

    }

    /**
     * 设置页面切换 1:历史记录 2:搜索关键字 3:搜索结果列表 4:全部搜索结果
     */
    ...
    private void showInput(View view) {
        ...
        ((InputMethodManager) getSystemService(INPUT_METHOD_SERVICE)).
                hideSoftInputFromWindow(SearchListActivity1.this.getCurrentFocus().getWindowToken(),
...
//        }
    ...
    private void setData2(String search_key) {
        ...
        if (fgrxxBean != null) {
            presenter.getNew1SearchData1(search_key, fgrxxBean.getUserId(), fgrxxBean.getName(), "-1", "3519379523491604669", 2);
        }
//        presenter.getNew1SearchData(search_key, "99", 2);
    ...
    private void setData3(String search_key) {
        ...
        FgrxxBean fgrxxBean = MmkvUtils.getInstance().get_common_json(AppCommonUtils.userInfo, FgrxxBean.class);
        if (fgrxxBean != null) {
            presenter.getNew1SearchData1(search_key, fgrxxBean.getUserId(), fgrxxBean.getName(), "-1", "3519379523491604669", 3);
        }
    }

    /*new搜索接口*/
    @Override
    public void OnNewSearchListSuccess(SNewListBean1 bean, int i) {
        if (i == 2) {
            mAdapter2.setNewData(bean.getList());
            if (mList.size() > 0) {
                set_ll_vis(2);
            }
        }
        if (i == 3) {
            mAdapter4.setNewData(bean.getList());
            if (mList.size() == 0) {
    ...
    @Override
    public void OnNewSearchListNodata(String bean, int i) {
        if (i == 3) {
            niubiEmptyView3.nodata("暂无数据");
        }
    }
    ...
    @Override
    public void OnNewSearchListFail(String msg, int i) {
        if (i == 3) {
