...
public class PlayloadDelegate {
    ...
    private final static String MOB_PUSH_NORMAL_PLAYLOAD_KEY = "pluginExtra";
    ...
    public void playload(Context context, Bundle bundle) {
        ...
        try {
            ...
            for (String key : keySet) {
                Log.e("MobPush playload bundle------------->key", key);
                Log.e("MobPush playload bundle------------->bundle.get(key)", bundle.get(key).toString());
...
//
//                } else
                if (key.equals(MOB_PUSH_NORMAL_PLAYLOAD_KEY)) {

                    map = parseNormalPlayload(bundle);

                } else {
                    Object object = bundle.get(key);
                    Log.e("MobPush playload bundle------------->结果", ">>>>>>key: " + key + ", object: " + object);
    ...
    private void realPerform(Context context, HashMap<String, Object> map) {
        ...
        for (String key : keys) {
            Log.e("MobPush playload bundle------------->realPerform", ">>>>>>>>>>all key: " + key + ", value: " + map.get(key));
        }
        ...
        Log.e("MobPush playload bundle------------->realPerform", json);
...
//                openUrl(context, map);
//            } else
            if (map.containsKey(MOB_PUSH_NORMAL_SCHEME_PLAYLOAD_KEY)) {
    ...
    private HashMap<String, Object> parseOPPOPlayload(Bundle bundle) {
        ...
        Log.e("MobPush playload bundle------------->hashMap", hashMap.size() + "");
        return hashMap;
    }
