 ...
 * 咨询展示2 可以展示ViewPager
 */
class RecommandFragment : SlbBaseFragmentViewPageYewuList(), ClassificationByPageView {

    private var mPresenter: ClassificationByPagePresenter? = null

    companion object {
        fun getInstance(bean: BjyyBeanYewu3): RecommandFragment {
            val recommandFragment = RecommandFragment()
            val bundle = Bundle()
            bundle.putSerializable("feileiBean", bean)
            recommandFragment.arguments = bundle
