...
public class MusicPlayerUtil {
        ...
        for (int i = 0; i < files.length; i++) {
            if (!files[i].isDirectory()) {        // 判断是否为文件夹
                ...
                if (fileName.trim().toLowerCase().endsWith(type)) {   // 判断文件类型是否为type
                    ...
                    String[] music = {fileName, MusicPlayerUtil.format(mediaPlayerDuration)};
    ...
    /**
     * 获取上一首歌的下标，-1表示列表播放到第一首了
     *
     * @param nowPosition   当前歌曲的下标
     * @param playMode      播放模式    0顺序，1循环，2随机，3单曲
     * @param musicListSize 歌单的歌曲数
     * @return
     */
    public static int getPreMusicPosition(int nowPosition, int playMode, int musicListSize) {
        int nextPosition = -1;
        switch (playMode) {
            case 0:
                if (nowPosition - 1 <= 0) {
