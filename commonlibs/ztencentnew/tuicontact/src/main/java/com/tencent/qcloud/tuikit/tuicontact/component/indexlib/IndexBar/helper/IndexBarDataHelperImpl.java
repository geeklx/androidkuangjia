...
//                    pySb.append(Pinyin.toPinyin(target.charAt(i1)).toUpperCase());
                    if (!TextUtils.isEmpty(mPinYinUtils.getCharPinYin(target.charAt(i1)))) {
                        pySb.append(mPinYinUtils.getCharPinYin(target.charAt(i1)).toUpperCase());
                    }
                }
    ...
    public IIndexBarDataHelper fillInexTag(List<? extends BaseIndexPinyinBean> datas) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (indexPinyinBean.isNeedToPinyin()) {
                //以下代码设置城市拼音首字母
                if (indexPinyinBean.getBaseIndexPinyin().length()>0){
