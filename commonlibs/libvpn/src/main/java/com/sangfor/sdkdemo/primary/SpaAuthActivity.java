...
import com.sangfor.sdkdemo.view.SpaConfigActivity;


public class SpaAuthActivity extends AppCompatActivity implements SFAuthResultListener, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.vpn_activity_spa_auth);
        //加载UI
        initView();
        //加载上次登录信息
        setLoginInfo();
    ...
    private void startAutoTicket() {
        ...
        if (SFUemSDK.getInstance().startAutoTicket()){
            showToast("免密成功");
            startActivity(new Intent(SpaAuthActivity.this, AuthSuccessActivity.class));
            finish();
        }
    }
    ...
    private void initView() {
        ...
        mSpaConfigButton = findViewById(R.id.set_spa_button);
        mSpaConfigButton.setOnClickListener(this);
    }

    @Override
    public void onClick(View v) {
        int id = v.getId();
        if (id == R.id.svpn_login_button) {
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                if (mProgressDialog == null || !mProgressDialog.isShowing()) {
                    mProgressDialog = new ProgressDialog(SpaAuthActivity.this);
                    mProgressDialog.setProgressStyle(ProgressDialog.STYLE_SPINNER);
                    mProgressDialog.setTitle("");
                    mProgressDialog.setMessage(SpaAuthActivity.this.getString(R.string.waiting));
    ...
    public void showErrorMessage(final String errMsg) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Dialog messageDialog = new AlertDialog.Builder(SpaAuthActivity.this)
                        .setTitle(SpaAuthActivity.this.getString(R.string.info))
                        .setMessage(errMsg)
    ...
    public void showToast(final String text) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(SpaAuthActivity.this, text, Toast.LENGTH_SHORT).show();
            }
        });
    }
